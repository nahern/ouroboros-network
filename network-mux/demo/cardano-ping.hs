...
main = do
    ...
    (addresses, versions) <- case unixSock options of
        Nothing -> do
          addrs <- Socket.getAddrInfo (Just hints) (host options) (Just $ port options)
          return (addrs, supportedNodeToNodeVersions $ magic options)
        Just fname ->
          return ([ Socket.AddrInfo [] Socket.AF_UNIX Socket.Stream Socket.defaultProtocol
                    (Socket.SockAddrUnix fname) Nothing ]
                 , supportedNodeToClientVersions $ magic options)

    laid <- async $ logger msgQueue $ json options
    caids <- mapM (async . pingClient (Tracer $ doLog msgQueue) options versions ) addresses
    mapM_ wait caids
  ...
  where
    ...
    decodeVersion = do
        ...
        case version of
             ...
             32773 -> Right . NodeToClientVersionV5 <$> CBOR.decodeWord32
             32774 -> Right . NodeToClientVersionV6 <$> CBOR.decodeWord32
             32775 -> Right . NodeToClientVersionV7 <$> CBOR.decodeWord32
             32776 -> Right . NodeToClientVersionV8 <$> CBOR.decodeWord32
  ...
  where
    ...
    peerString =
        case Socket.addrFamily peer of
             ...
             _ -> do
                  (Just host, Just port) <- Socket.getNameInfo [Socket.NI_NUMERICHOST
