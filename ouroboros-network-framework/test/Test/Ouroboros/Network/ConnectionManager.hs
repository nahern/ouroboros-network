  ...
  where
    ...
    close (FD v) =
      uninterruptibleMask_
        $ atomically
        $ modifyTVar v (\fds -> fds { fdConnectionState = ClosedState })
...
mkConnectionHandler conns = do
    ...
    return $ ConnectionHandler {
        connectionHandler =
          ...
          (\v _ ConnectionId { remoteAddress } _ ->
            Action $ \unmask -> unmask $ do
              labelThisThread ("outbound-handler-"
          ...
          (\v _ ConnectionId { remoteAddress } _ ->
            Action $ \unmask -> unmask $ do
              labelThisThread ("inbound-handler-"
                                 ++ show (getPeerAddr (getTestAddress remoteAddress)))
              conn <-
                atomically $ do
...
prop_connectionManager myAddress' (Connections conns) =
    let tr = runSimTrace experiment
        ...
        numberOfReused =
          ...
          . filter (\msg ->
                   case msg of
                     TrReusedConnection {}     -> True
                     TrReusedIdleConnection {} -> True
                     _                         -> False)
          $ cmTrace
    in
          ...
          let -- reconstruct list of connections from the trace
              ...
              fromTrace as (TrReusedConnection connId : tr') =
                fromTrace
                  ...
                  tr'
              fromTrace as (TrReusedIdleConnection connId : tr') =
                fromTrace
                  (( getTestAddress (remoteAddress connId)
                   , Outbound
                   , Just Duplex
    ...
    experiment = do
        ...
        withConnectionManager
          ConnectionManagerArguments {
              ...
              cmTimeWaitTimeout = defaultTimeWaitTimeout
            }
            connectionHandler
            (\_ -> HandshakeFailure)
                          ...
                          do
                            ...
                            _ <-
                              unregisterOutboundConnection
                                connectionManager addr
                            return ()
                          `catch` \(e :: ConnectionManagerError Addr) ->
                            case e of
                              ForbiddenConnection {} -> return ()
                              _ -> throwIO e
...
cmtPeerAddr (TrReusedIdleConnection connId)     = Just (remoteAddress connId)
cmtPeerAddr (TrTerminatingConnection _ connId)  = Just (remoteAddress connId)
cmtPeerAddr (TrTerminatedConnection _ peerAddr) = Just peerAddr
cmtPeerAddr (TrConnectionHandler connId _)      = Just (remoteAddress connId)
cmtPeerAddr  TrShutdown                         = Nothing
cmtPeerAddr (TrConnectionExists _ peerAddr _)   = Just peerAddr
...
cmtPeerAddr (TrDemotedToColdLocal peerAddr _)   = Just peerAddr
cmtPeerAddr (TrPruneConnections _)              = Nothing
cmtPeerAddr (TrConnectionCleanup connId)        = Just (remoteAddress connId)
cmtPeerAddr (TrConnectionTimeWait connId)       = Just (remoteAddress connId)
cmtPeerAddr (TrConnectionTimeWaitDone connId)   = Just (remoteAddress connId)
cmtPeerAddr (TrConnectionTerminated connId _)   = Just (remoteAddress connId)
cmtPeerAddr  TrDebugState {}                    = Nothing
