...
data ScheduleEntry extra =
    ...
    -- @
    --
    -- 'seStart' is a delay from when last connection was scheduledr;
...
-- | Linearized schedule of inbound connections.
--
...
-- runtime, i.e. for 'connect' and 'accept' we use 'IOExceptions'.
--
data SnocketError =
...
-- 'precision'), as we cannot guarante that events in the same time slot will
...
--
--  The purpose of this simulation is to put the conneciton manager state
        ...
        withConnectionManager
          ...
          $ \(connectionManager
            ...
            forever (atomically (ControlChannel.readMessage inbgovControlChannel) $> ())
              `race_`
              (do threads <- go [] (accept snocket fd) (schedule scheduleMap)
...
-- Unfortuntatelly, there's no way to simulate @TimeExpired@ followed by
-- @PromotedToWarm^{Duplex}_{Remote}@.
--
