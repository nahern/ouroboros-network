  ...
  where
    ...
    reqRespInitiator protocolNum requestsVar =
      InitiatorProtocolOnly
        (MuxPeer
          ((name,"Initiator",protocolNum,) `contramap` debugTracer) -- TraceSendRecv
          codecReqResp
          (Effect $ do
            reqs <-
              atomically $ do
  ...
  where
    ...
    reqRespInitiatorAndResponder protocolNum fn accInit requestsVar =
      InitiatorAndResponderProtocol
        ...
        (MuxPeer
          ((name,"Responder",protocolNum,) `contramap` debugTracer) -- TraceSendRecv
          codecReqResp
            ...
            (rs :: [Either SomeException (Bundle [resp])]) <-
                replicateM
                  ...
                  (bracket
                     ...
                     (\connHandle -> do
                      case connHandle of
                        Connected _ _ (Handle mux muxBundle _ _ :: Handle InitiatorMode peerAddr ByteString m [resp] Void) ->
                          try @_ @SomeException $
                            (runInitiatorProtocols
...
prop_unidirectional_IO clientAndServerData compatMode =
    ioProperty $ do
      print ("START", clientAndServerData)
      withIOManager $ \iomgr ->
        bracket
