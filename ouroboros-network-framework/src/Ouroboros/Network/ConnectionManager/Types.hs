...
newtype Action m a = Action {
    runActionWithUnmask :: (forall x. m x -> m x) -> m a
  }
...
resultInState :: OperationResult InState -> InState
resultInState (UnsupportedState inState) = inState
resultInState (OperationSuccess inState) = inState
...
data OutboundConnectionManager (muxMode :: MuxMode) socket peerAddr handle handleError m where
    OutboundConnectionManager
      ...
      => { ocmRequestConnection    :: RequestOutboundConnection peerAddr handle handleError m
         , ocmUnregisterConnection :: peerAddr -> m (OperationResult InState)
         }
...
data InboundConnectionManager (muxMode :: MuxMode) socket peerAddr handle handleError m where
    InboundConnectionManager
      ...
      => { icmIncludeConnection    :: IncludeInboundConnection socket peerAddr handle handleError m
         ...
         , icmPromotedToWarmRemote :: peerAddr -> m (OperationResult InState)
...
data InState
    ...
    | InOutboundUniState
    | InOutboundDupState !TimeoutExpired
    | InDuplexState
...
data ConnectionManagerTrace peerAddr handlerTrace
  ...
  | TrReusedIdleConnection       !(ConnectionId peerAddr)
  ...
  | TrShutdown
  | TrConnectionExists           !Provenance !peerAddr    !InState
  ...
  | TrDemotedToColdLocal         !peerAddr                !InState
  ...
  | TrConnectionTerminated       !(ConnectionId peerAddr) !InState
  | TrDebugState                 !(Map peerAddr InState)
  deriving Show
