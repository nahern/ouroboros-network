...
withConnectionManager ConnectionManagerArguments {
  ...
  where
      ...
      let cleanup :: m ()
          cleanup =
            ...
            uninterruptibleMask $ \unmask -> do
              ...
              mConnVar <- modifyTMVar stateVar $ \state -> do
                ...
                case wConnVar of
                  Nowhere -> do
                    ...
                    return ( state
                           , Left UnknownConnection
                           )
                  There connState -> do
                    close cmSnocket socket
                    return ( Map.delete peerAddr state
                           , Left (summariseState connState)
                           )
                  Here connVar -> do
                    close cmSnocket socket
                    return ( state
              ...
              case mConnVar of
                Left !inState ->
                  traceWith tracer (TrConnectionTerminated connId inState)
                Right connVar ->
                  ...
                  `finally` do
                    ...
                    inState <- atomically $ do
                      mConnState <- readTMVar stateVar
                                >>= traverse readTVar . Map.lookup peerAddr
                      -- We can always write to `connVar`, a new connection
                      -- will use a new 'TVar', but we can only delete it
                      -- from 'ConnectionManagerState' if it is in
                      -- 'TerminatingState'.  It might happen that in
                      -- 'TerminatingState' the server will accept
                      -- a connection.  It will have a new 'TVar' and insert
                      ...
                      case mConnState of
                        ...
                        _ ->
                          return ()
                      return (maybe UnknownConnection summariseState mConnState)
                    traceWith tracer (TrConnectionTerminated connId inState)
    ...
    unregisterOutboundConnectionImpl stateVar peerAddr = do
      ...
      case transition of
        DemotedToColdLocal _connId connThread inState -> do
          traceWith tracer (TrDemotedToColdLocal peerAddr inState)
          cancel connThread
        ...
        PruneConnections _connId pruneMap -> do
          traceWith tracer (TrDemotedToColdLocal peerAddr (InInboundState Duplex))
        ...
        DemoteToColdLocalNoop inState -> do
          traceWith tracer (TrDemotedToColdLocal peerAddr inState)
          return (OperationSuccess inState)
