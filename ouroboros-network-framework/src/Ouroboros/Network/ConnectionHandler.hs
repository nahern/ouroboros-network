  ...
  where
    ...
    outboundConnectionHandler
      ...
      => ConnectionHandlerFn (ConnectionHandlerTrace versionNumber versionData)
                             ...
                             (versionNumber, versionData)
                             m
    outboundConnectionHandler PromiseWriter { writePromise }
                              tracer
                              connectionId@ConnectionId { remoteAddress }
                              muxBearer
        = Action { runActionWithUnmask }
      where
        runActionWithUnmask :: (forall x. m x -> m x) -> m ()
        runActionWithUnmask unmask =
          classifyExceptions tracer remoteAddress OutboundError $ do
            hsResult <-
              unmask $ runHandshakeClient (muxBearer sduHandshakeTimeout)
                                          connectionId
                                          handshakeArguments
    ...
    inboundConnectionHandler
      ...
      => ConnectionHandlerFn (ConnectionHandlerTrace versionNumber versionData)
                             ...
                             (versionNumber, versionData)
                             m
    inboundConnectionHandler PromiseWriter { writePromise }
                             tracer
                             connectionId@ConnectionId { remoteAddress }
                             muxBearer
        = Action runActionWithUnmask
      where
        ...
        runActionWithUnmask unmask =
          classifyExceptions tracer remoteAddress InboundError $ do
            hsResult <-
              unmask $ runHandshakeServer (muxBearer sduHandshakeTimeout)
                                          connectionId
                                          handshakeArguments
