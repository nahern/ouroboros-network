          ...
          RemoteIdle timeoutSTM ->
            if csCompatMode
              then
                ...
                -- restarting hot and warm protocols in 'RemoteIdle` state
                -- will not work.   We choose, what we think, is the simplest
                -- solution, just wait on `timeoutSTM` or mux termination before
                -- sealing 'CommitRemote'.  During this timeout the other STM
                -- transaction might discover traffic on this connection and do
                -- the @PromoteToWarm^{Unidirectional}_{Remote}@.  Note that
                -- a non timely termination should be considered a protocol
                -- error, we will not treat it as such.
                    outer
    ...
    inboundGovernorLoop !state = do
      ...
      case event of
                          ...
                          case mCompletionMap of
                            ...
                            Just csCompletionMap -> do
                              ...
                              let -- initial state is 'RemoteIdle', if the remote end will not
                                  ...
                                  -- static set of established mini-protocols ids
                                  csEstablishedSet :: Set MiniProtocolNum
                                  csEstablishedSet =
                                    case muxBundle of
        ...
        AwakeRemote connId -> do
          ...
          assert (resultInState res /= UnknownConnection) $
            inboundGovernorLoop
              (updateRemoteState connId
                                 RemoteEstablished
                                 state)
...
data ServerTrace peerAddr
    ...
    | TrPromotedToWarmRemote        !(ConnectionId peerAddr) !(OperationResult InState)
    ...
    | TrWaitIdleRemote              !(ConnectionId peerAddr) !(OperationResult InState)
