...
import           Control.Monad.Class.MonadSTM.Strict (newTVarIO, readTVar, MonadSTM (atomically), writeTVar, StrictTVar)
...
mockDNSActions dnsMap stdGenVar = dnsActions
 where
   genDiffTime :: Integer -> Integer -> StdGen -> DiffTime
   genDiffTime lo hi = picosecondsToDiffTime . fst . uniformR (lo * 1000000000, hi * 1000000000)
   ...
   dnsLookupAWithTTL timeout _ _ domain = do
     ...
     atomically @(IOSim s) $ writeTVar stdGenVar g'
   ...
   dnsActions :: DNSActions () exception (IOSim s)
...
--
mockPublicRootPeersProvider :: forall s. MockRoots -> Int -> IOSim s (Set SockAddr, DiffTime)
