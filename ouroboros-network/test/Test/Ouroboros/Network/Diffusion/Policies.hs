...
newtype ArbitraryKnownPeerInfo = ArbitraryKnownPeerInfo KnownPeerInfo
                                 deriving Show

instance Arbitrary ArbitraryKnownPeerInfo where
    arbitrary = do
        tepid <- arbitrary
        ...
        return $ ArbitraryKnownPeerInfo $ KnownPeerInfo failCnt tepid


data ArbitraryPolicyArguments = ArbitraryPolicyArguments {
    apaAvailable     :: Map SockAddr KnownPeerInfo
  , apaPickNum       :: Int
    ...
    arbitrary = do
     ...
     where
       ...
       fn :: ArbitrarySockAddr
          -> ArbitraryKnownPeerInfo
          -> (SockAddr, KnownPeerInfo)
       fn (ArbitrarySockAddr addr) (ArbitraryKnownPeerInfo kpi) = (addr, kpi)
...
prop_randomDemotionM ArbitraryPolicyArguments{..} seed = do
  ...
  where
    ...
    doDemotion 0 _ countMap = do
      ...
      where
        kpiFilter :: KnownPeerInfo -> Bool
        kpiFilter = case apaDemotion of
                     ArbitraryWarmDemotion -> knownPeerTepid
                     ArbitraryColdDemotion ->
                         (\kpi -> knownPeerFailCount kpi > 0)
